/**************************************************************************
      ...
      Author:   Bruce E. Hall, w8bh.net
        Date:   13 Feb 2021
    Hardware:   HiLetGo ESP32 or D1 Mini ESP8266, ILI9341 TFT display OR:
              WA2FZW & VK3PE  Also for CYD display (ILI9341)
              Add this to User_setup_select.h file (dispable any other active)
              //CYD ESP32   ~June 2025
              //#include <User_Setups/ESP32_Cheap_Yellow_Display.h> 		//John Price mod for CYD June 30th '25
              // and the above file in User_setups Directory file.
...
#include <WiFi.h>                                  //w4oa // use this WiFi lib for ESP32
#define TITLE              "NTP TIME"
#define WIFI_SSID          "records"               
#define WIFI_PWD           "huxl3yabby"      
 

#define NTP_SERVER         "pool.ntp.org"            // time.nist.gov, time.apple.com, etc
//#define TZ_RULE	"EST5EDT,M3.2.0/2:00:00,M11.1.0/2:00:00"		// John - US Eastern time
//#define	TZ_RULE	"AEST-10AEDT,M10.1.0/2:00:00,M4.1.0/2:00:00" 	// Glenn - Aus Eastern time
//#define	TZ_RULE	"GMT0BST,M3.5.0/1:00:00,M10.5.0/2:00:00"		// Jim - England
	#define TZ_RULE "PST8PDT,M3.2.0/2:00:00,M11.1.0/2:00:00"  		// USA West Coast Time (Olympia, WA)
//#define TZ_RULE            "EST5EDT,M3.2.0/2:00:00,M11.1.0/2:00:00"
//#define TZ_RULE            "CST6CDT,M3.2.0/2:00:00,M11.1.0/2:00:00"
//#define TZ_RULE            "MST7MDT,M3.2.0/2:00:00,M11.1.0/2:00:00"
//#define TZ_RULE            "PST8PDT,M3.2.0/2:00:00,M11.1.0/2:00:00"

#define SCREEN_ORIENTATION       1                 // screen landscape mode: 1=cable right, 3=cable left
#define LED_PIN                 17    // 2        //w4oa // built-in LED is on GPIO pins 4 red, 16 green, 17 blue                // built-in LED is on GPIO 2
...
#define C6S_WHITE         0xF79D                   // #f5f0e8 - warm white headings
#define C6S_SLATE_LIGHT   0xD657                   // #d4cab9 - body text
#define C6S_NAVY_MID      0x4A49                   // #504a52 - borders

#define TIMECOLOR       C6S_ACCENT                 // color of 7-segment time display
#define DATECOLOR       C6S_SLATE_LIGHT            // color of displayed month & day
#define LABEL_FGCOLOR   C6S_WHITE                  // color of label text
#define LABEL_BGCOLOR   C6S_NAVY_LIGHT             // color of label background
...
void showClockStatus() {
  ...
  tft.setTextColor(C6S_NAVY,color);
...
void showTime(time_t t, bool hr12, int x, int y) {
  ...
  tft.setTextColor(TIMECOLOR, C6S_NAVY);          // set time color
  ...
  if (h<10) {                                      // is hour a single digit?
    if ((!hr12)||(HOUR_LEADING_ZERO))              // 24hr format: always use leading 0
      ...
      tft.setTextColor(C6S_NAVY,C6S_NAVY);       // black on black text     
      x+=tft.drawChar('8',x,y,f);                  // will erase the old digit
      tft.setTextColor(TIMECOLOR,C6S_NAVY);      
    }
  }
...
void showDate(time_t t, int x, int y) {
  ...
  const char* months[] = {"JAN","FEB","MAR",
     ...
     "NOV","DEC"};
  tft.setTextColor(DATECOLOR, C6S_NAVY);
  int i=0, m=month(t), d=day(t);                   // get date components 
  tft.fillRect(x,y,50,60,C6S_NAVY);               // erase previous date
...
void newDualScreen() {
  tft.fillScreen(C6S_NAVY);                        // start with C6S background
  ...
  tft.drawRoundRect(0,0,319,110,10,C6S_NAVY_MID);  // draw edge around local time
  tft.drawRoundRect(0,126,319,110,10,C6S_NAVY_MID);// draw edge around UTC
}

void startupScreen() {
  tft.fillScreen(C6S_NAVY);                        // start with C6S background
  tft.fillRoundRect(0,0,319,30,10,LABEL_BGCOLOR);  // title bar
  tft.drawRoundRect(0,0,319,239,10,C6S_NAVY_MID);  // draw edge screen
  tft.setTextColor(LABEL_FGCOLOR,LABEL_BGCOLOR);   // set label colors
  tft.drawCentreString(TITLE,160,2,4);             // show sketch title on screen
  tft.setTextColor(LABEL_FGCOLOR, C6S_NAVY);       // set text color
}
...
void blink(int count=1) {                          // diagnostic LED blink
  ...
  for (int i=0; i<count; i++) {                    // blink counter
    digitalWrite(LED_PIN,0);                       //w4oa flash blue led 3 times                      // turn LED on 
    delay(200);                                    // for 0.2s
...
void setup() {
  ...
  tft.invertDisplay(false);                         // w4oa true or false: invert the red and blue channels
                                                   //    to correct the color order
